...
/**
 * 누구와 같은 편이었고, 그때 몇 번 이겼는가.
 *
 * 리폿에 남은 승리 팀·패배 팀 명단을 그대로 읽는다. 늦은 참석자는 어느
 * 편도 아니어서 세지 않는다. 취소된 경기도 뺀다 — 점수가 되돌려진 경기다.
 */
function mateStats(handle) {
  const map = new Map();
  ...
  return [...map.values()].map(st => ({
    ...
    pct: Math.round((st.wins / st.games) * 1000) / 10,
  }));
}
 ...
 * 알 수가 없다.
 */
function bestMates(handle) {
  const all = mateStats(handle);
...
/** 포인트 순위. 같은 점수는 같은 등수다. */
function pointRank(handle) {
  const list = viewPlayers();
  const p = list.find(x => x.handle === handle);
  if (!p) return null;
  const above = list.filter(x => x.point > p.point).length;
  const tied = list.filter(x => x.point === p.point).length;
  return { rank: above + 1, tied: tied > 1, of: list.length };
}

/** 이름을 누르면 열리는 한 장. */
function showPlayerCard(handle) {
  const p = viewPlayers().find(x => x.handle === handle);
  if (!p) {
    openModal(`<button class="modal-x" onclick="closeModal()" aria-label="닫기">&times;</button>
      <h3>${esc(handle)}</h3>
      <div class="pc-empty">이 달 명단에 없는 아이디입니다.</div>`);
    return;
  }

  const games = p.wins + p.losses;
  const r = pointRank(handle);
  const mates = bestMates(handle);
  const cur = viewSeason();
  // 시즌 이름은 "2026년 8월 27일 ~ 9월 30일" 처럼 길어 칸을 두 줄로 밀어낸다.
  // 달 이름 한 줄이면 어느 달 기록인지는 충분히 전해진다.
  const monthText = (monthOptions().find(o => cur && o.id === cur.id) || {}).text || '';
  ...
  openModal(`
    <button class="modal-x" onclick="closeModal()" aria-label="닫기">&times;</button>
    <div class="pc-head">
      <div class="pc-id">${esc(p.handle)}</div>
      <div class="pc-clan">${esc(p.clan && p.clan !== '-' ? p.clan : '무소속')}</div>
    </div>

    <div class="pc-grid">
      <div class="pc-cell">
        <div class="pc-k">RANK</div>
        <div class="pc-v"><b>${r ? r.rank : '-'}</b><span class="pc-u">위</span></div>
        <div class="pc-sub">${r ? `${r.of}명 중${r.tied ? ' · 동점' : ''}` : ''}</div>
      </div>
      <div class="pc-cell">
        <div class="pc-k">POINT</div>
        <div class="pc-v"><b>${p.point}</b><span class="pc-u">점</span></div>
        <div class="pc-sub">${esc(monthText)}</div>
      </div>
      <div class="pc-cell">
        <div class="pc-k">전적</div>
        <div class="pc-v pc-rec"><b>${games}</b><span class="pc-u">전</span>
          <b class="w">${p.wins}</b><span class="pc-u">승</span>
          <b class="l">${p.losses}</b><span class="pc-u">패</span></div>
      </div>
      <div class="pc-cell">
        <div class="pc-k">승률</div>
        <div class="pc-v"><b>${ratio(p)}</b><span class="pc-u">%</span></div>
        <div class="pc-bar"><div class="pc-bar-fill" style="width:${games ? ratio(p) : 0}%"></div></div>
      </div>
    </div>
