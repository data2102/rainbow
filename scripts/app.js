...
function renderRoleTable() {
  ...
  tbody.innerHTML = list.map(a => `<tr>
    ...
    <td><span class="acct-actions">
      ...
      ${me && me.handle === a.handle
        ? ''
        : `<button class="btn-mini btn-reject" data-adel="${esc(a.handle)}">삭제</button>`}
    </span></td>
  </tr>`).join('') || '<tr><td colspan="5" class="log-empty">계정이 없습니다.</td></tr>';
}
...
function renderAcctLogs() {
  ...
  tbody.innerHTML = acctLogs.map(l => {
    const d = l.detail || {};
    const target = d.handle || d.to || d.target || '-';
    let extra = '';
    if (l.action === 'set_role' && d.from) {
